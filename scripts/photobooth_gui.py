...
root.bind('<Escape>', lambda e: e.widget.quit()) # BH added

## boothcam may need to present a file dialog gui. So import after root is defined.
...
Button_enabled = False # BH can be deleted???
...
def check_and_snap(force=False, countdown1=None):
    ...
    if (Button_enabled == False):
        ...
        Button_enabled = True
#        can.delete("text")
        can.create_text(WIDTH/2, HEIGHT - STATUS_H_OFFSET, text="Touch screen when ready", font=custom.CANVAS_FONT, tags="text", fill="green") # changed text, added fill
#        can.update()
...
#        setLights(r_var.get(), g_var.get(), b_var.get())
        if im is not None:
            ...
            display_image(im)
            can.delete("text")
            can.create_text(WIDTH/2, HEIGHT - STATUS_H_OFFSET, text="Uploading Image", font=custom.CANVAS_FONT, tags="text", fill="green") # added fill
            can.update()
            if signed_in:
...
frame = Frame(root, bg="black") # this is the frame of the widget on the top of the screen
#Button(frame, text="Exit", font=custom.BUTTON_FONT, command=on_close).pack(side=LEFT)
#Button(frame, text="Customize", font=custom.BUTTON_FONT, command=lambda *args: custom.customize(root)).pack(side=LEFT)
tkkb_button = Button(frame, command=launch_tkkb, text="Launch-KB")
# tkkb_button.pack(side=LEFT)
send_button = Button(frame, text="Email", command=sendPic, font=custom.BUTTON_FONT) # changed
...
etext = Entry(frame, width=40, textvariable=email_addr, font=custom.BUTTON_FONT) # removed ', font=custom.BUTTON_FONT
etext.pack(side=LEFT) # added side=LEFT to center vertically
frame.pack()
...
can = Canvas(root, width=WIDTH, height=HEIGHT, background='black', bd=0) # added background property
can.pack()
...
can.delete("text")
can.create_text(WIDTH/2, HEIGHT/2, text="SMILE ;-)", font=custom.CANVAS_FONT, tags="splash", fill="yellow")
can.update()
